...
## MUMU Hermes integration

When this skill runs inside MUMU Hermes, `delegate_task` satisfies the subagent requirement. In Expert mode, an explicit request to finish the whole deck authorizes the outline, style, backend, and sample gates to be recorded as decisions under the deck project and continued autonomously unless the user asked to approve them interactively. Never silently choose an unavailable image backend: run its capability check first, and fall back to the editable `powerpoint` skill when image generation is unavailable.

Render and inspect every slide before delivery. Save every final slide preview under the deck project, include all previews in `quality-submission.json`, and emit an artifact marker for the deck, the quality report, and each final preview. A slide is not complete merely because the PPTX package opens.
