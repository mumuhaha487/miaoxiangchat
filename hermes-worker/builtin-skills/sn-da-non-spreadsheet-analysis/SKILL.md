...
## MUMU Runtime Adaptation

- Start with `document-tool inspect SOURCE --ocr auto`; it uses the installed Office, PDF, and OCR toolchain and avoids assumptions about unavailable SenseNova-only helpers.
- This installation does not include `sn-da-image-caption`. Do not invoke `caption.py` unless it is actually present. For image-only pages or slides, render them to PNG, inspect them with the available visible-browser/vision tools, and save requested screenshots under `/workspace` as verified artifacts.
- If visual inspection is unavailable, return the extracted text and clearly identify the pages that still require visual review. Never invent image or chart content.
