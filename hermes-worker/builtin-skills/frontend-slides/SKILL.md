...
## MUMU Runtime Safety and Verification

- Use the built-in visible browser to open and visually inspect the generated deck. Verify every slide at desktop size and at least one phone viewport; DOM or `scrollHeight` checks alone are insufficient.
- When the user requests screenshots, or screenshots are needed as delivery evidence, save the browser captures under `/workspace` and include them as verified artifacts.
- Do not install Vercel tooling, log into an external account, upload files, or publish a deck unless the user explicitly requests that external action in the current task. Generating or previewing a deck is not authorization to publish it.
...
Only enter this phase when the user explicitly requests sharing, deployment, or PDF export. Do not proactively publish or ask the user to authorize an external deployment as part of ordinary delivery.
